...
public extension UIButton {
    ...
    public func hnk_setImage(image: UIImage, key: String, state: UIControlState = [], placeholder: UIImage? = nil, format: Format<UIImage>? = nil, success succeed: ((UIImage) -> ())? = nil) {
    ...
    public func hnk_setImageFromFile(path: String, state: UIControlState = [], placeholder: UIImage? = nil, format: Format<UIImage>? = nil, failure fail: ((NSError?) -> ())? = nil, success succeed: ((UIImage) -> ())? = nil) {
        let fetcher = DiskFetcher<UIImage>(path: path)
        self.hnk_setImageFromFetcher(fetcher: fetcher, state: state, placeholder: placeholder, format: format, failure: fail, success: succeed)
    }
    
    public func hnk_setImageFromFetcher(fetcher: Fetcher<UIImage>, state: UIControlState = [], placeholder: UIImage? = nil, format: Format<UIImage>? = nil, failure fail: ((NSError?) -> ())? = nil, success succeed: ((UIImage) -> ())? = nil){
        self.hnk_cancelSetImage()
        self.hnk_imageFetcher = fetcher
        
        let didSetImage = self.hnk_fetchImageForFetcher(fetcher: fetcher, state: state, format : format, failure: fail, success: succeed)
    ...
    func hnk_fetchImageForFetcher(fetcher : Fetcher<UIImage>, state : UIControlState = [], format : Format<UIImage>? = nil, failure fail : ((NSError?) -> ())?, success succeed : ((UIImage) -> ())?) -> Bool {
        let format = format ?? self.hnk_imageFormat
        let cache = Shared.imageCache
        
        if let _ = cache.formats[format.name] {
            cache.addFormat(format: format)
        }
        var animated = false
        let fetch = cache.fetch(fetcher: fetcher, formatName: format.name, failure: {[weak self] error in
            if let strongSelf = self {
                if strongSelf.hnk_shouldCancelImageForKey(key: fetcher.key) { return }
            ...
            if let strongSelf = self {
                ...
                strongSelf.hnk_setImage(image: image, state: state, animated: animated, success: succeed)
            }
        }
        animated = true
    ...
    public func hnk_setBackgroundImage(image : UIImage, key: String, state : UIControlState = [], placeholder : UIImage? = nil, format : Format<UIImage>? = nil, success succeed : ((UIImage) -> ())? = nil) {
        let fetcher = SimpleFetcher<UIImage>(key: key, value: image)
        self.hnk_setBackgroundImageFromFetcher(fetcher: fetcher, state: state, placeholder: placeholder, format: format, success: succeed)
    }
    
    public func hnk_setBackgroundImageFromFile(path: String, state : UIControlState = [], placeholder : UIImage? = nil, format : Format<UIImage>? = nil, failure fail : ((NSError?) -> ())? = nil, success succeed : ((UIImage) -> ())? = nil) {
        let fetcher = DiskFetcher<UIImage>(path: path)
        self.hnk_setBackgroundImageFromFetcher(fetcher: fetcher, state: state, placeholder: placeholder, format: format, failure: fail, success: succeed)
    }
    
    public func hnk_setBackgroundImageFromFetcher(fetcher : Fetcher<UIImage>, state : UIControlState = [], placeholder : UIImage? = nil, format : Format<UIImage>? = nil, failure fail : ((NSError?) -> ())? = nil, success succeed : ((UIImage) -> ())? = nil){
        self.hnk_cancelSetBackgroundImage()
        self.hnk_backgroundImageFetcher = fetcher
        
        let didSetImage = self.hnk_fetchBackgroundImageForFetcher(fetcher: fetcher, state: state, format : format, failure: fail, success: succeed)
    ...
    func hnk_fetchBackgroundImageForFetcher(fetcher: Fetcher<UIImage>, state: UIControlState = [], format: Format<UIImage>? = nil, failure fail: ((NSError?) -> ())?, success succeed : ((UIImage) -> ())?) -> Bool {
        ...
        let fetch = cache.fetch(fetcher: fetcher, formatName: format.name, failure: {[weak self] error in
            if let strongSelf = self {
                if strongSelf.hnk_shouldCancelBackgroundImageForKey(key: fetcher.key) { return }
            ...
            if let strongSelf = self {
                if strongSelf.hnk_shouldCancelBackgroundImageForKey(key: fetcher.key) { return }
                strongSelf.hnk_setBackgroundImage(image: image, state: state, animated: animated, success: succeed)
            }
        }
        animated = true
        return fetch.hasSucceeded
